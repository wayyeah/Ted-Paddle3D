...
std::shared_ptr<paddle_infer::Predictor> create_predictor(
  ...
  std::cout<<"use_trt "<<use_trt<<std::endl;
  if (use_trt) {
    ...
    config.EnableTensorRtEngine(1 << 30, 1, 10, precision, trt_use_static,
                                false);
    std::cout<<"dynamic_shape_file "<<dynamic_shape_file<<std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::cout<< "preprocess time is: "<<static_cast<double>(end_time-start_time)/CLOCKS_PER_SEC<<"s"<<std::endl;//输出运行时间
  ...
  std::cout<< "Running time is: "<<static_cast<double>(end_time-start_time)/CLOCKS_PER_SEC<<"s"<<std::endl;//输出运行时间
