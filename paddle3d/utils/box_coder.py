...
class ResidualCoder(object):
    ...
    def encode_paddle(self, boxes, anchors):
        ...
        if len(cts)>0:
            return paddle.concat([xt, yt, zt, dxt, dyt, dzt, rts[0], cts], axis=-1)
        else:
            return paddle.concat([xt, yt, zt, dxt, dyt, dzt, rts[0]], axis=-1)
...
class PreviousResidualDecoder(object):
    ...
    @staticmethod
    def decode_paddle(box_encodings, anchors):
        """
        Args:
        ...
        """
        xa, ya, za, dxa, dya, dza, ra, *cas = paddle.split(anchors, 1, axis=-1)
        ...
        dxg = paddle.exp(lt) * dxa
        dyg = paddle.exp(wt) * dya
        dzg = paddle.exp(ht) * dza
        rg = rt + ra
