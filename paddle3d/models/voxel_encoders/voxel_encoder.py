...
__all__ = ['VoxelMean', 'HardVFE','VoxelMeanTed']
...
class VoxelMean(nn.Layer):
    def __init__(self, in_channels=4,model='max'):
...
@manager.VOXEL_ENCODERS.add_component
class VoxelMeanTed(nn.Layer):
    def __init__(self, in_channels=4,model='max'):
        super(VoxelMeanTed, self).__init__()
        self.in_channels = in_channels
        self.model=model
    ...
    def forward(self,batch_dict, coors=None):
        if 'rot_num' in batch_dict:
            rot_num=batch_dict['rot_num'][0]
        else:
            rot_num=1
        for i in range(rot_num):
            ...
            features=batch_dict['voxel_features'+frame_id]
            num_voxels=batch_dict['voxel_num_points'+frame_id]
            features_sum = paddle.sum(
                features[:, :, :self.in_channels], axis=1, keepdim=False)
            points_mean = features_sum / paddle.cast(
                num_voxels, features.dtype).reshape([-1, 1])
            ...
            if self.model is not None:
                if self.model == 'max':
                    time_max = paddle.max(features, axis=1, keepdim=False)
                    
                    points_mean[:, -1] = time_max[:, -1]
                    

            batch_dict['voxel_features'+frame_id] = points_mean
