...
from paddle3d.utils.bbloss import bb_loss
from paddle3d.models.common import box_utils
class RoIHeadBase(nn.Layer):
    ...
    def proposal_layer(self, batch_dict, nms_config):
        ...
        for index in range(batch_size):
            ...
            if nms_config['multi_class_nms']:
                ...
                selected_score, selected_label, selected_box,selected  = class_agnostic_nms(
    ...
    def assign_targets_ted(self, batch_dict, rot_num_id, enable_dif = False):
        batch_size = batch_dict['batch_size']
        with paddle.no_grad():
            if rot_num_id == 0:
                s_str = ''
        ...
        gt_of_rois = paddle.reshape(box_utils.rotate_points_along_z(points=paddle.reshape(gt_of_rois,[-1,1,gt_of_rois.shape[-1]]), angle=paddle.reshape(-roi_ry,[-1])),[batch_size, -1, gt_of_rois.shape[-1]])
        ...
        opposite_flag = (heading_label > np.pi * 0.5) & (heading_label <
                                                         np.pi * 1.5)
        if opposite_flag.numel() > 0 and opposite_flag.sum() > 0:
    ...
    def get_box_reg_layer_loss(self, forward_ret_dict):
        ...
        if loss_cfgs['reg_loss'] == 'smooth-l1':
            ...
            rois_anchor[:, 0:3] = 0
            rois_anchor[:, 6] = 0
            reg_targets = self.box_coder.encode_paddle(gt_boxes3d_ct.reshape([rcnn_batch_size, code_size]),rois_anchor)

            rcnn_loss_reg = self.reg_loss_func(rcnn_reg.reshape([rcnn_batch_size, -1]).unsqueeze(axis=0), reg_targets.unsqueeze(axis=0),)  # [B, M, 7]
            rcnn_loss_reg = (rcnn_loss_reg.reshape([rcnn_batch_size, -1]) *fg_mask.unsqueeze(axis=-1).astype('float32')).sum() / max(fg_sum, 1)
            rcnn_loss_reg = rcnn_loss_reg * loss_cfgs['loss_weights']['rcnn_reg_weight']
            tb_dict['rcnn_loss_reg'] = rcnn_loss_reg
            #print(rcnn_loss_reg.item())#大部分时间为0
        ...
        gt_boxes3d_ct = forward_ret_dict['gt_of_rois'].reshape([shape[0] * shape[1], -1])[:, 0:7]
        rcnn_reg = forward_ret_dict['rcnn_reg']  # (rcnn_batch_size, C)
        rois = forward_ret_dict['rois'].reshape([-1, code_size])[:, 0:7]
        rois[:, 0:3] = 0
        rois[:, 6] = 0

        batch_box_preds = self.box_coder.decode_paddle(rcnn_reg, rois).reshape([-1, code_size])
    ...
    def get_loss(self, tb_dict=None,batch_dict=None):
        tb_dict = {} if tb_dict is None else tb_dict
        
        for i in range(6):
            if 'targets_dict'+str(i) in batch_dict:
                self.forward_ret_dict['targets_dict'+str(i)]=batch_dict['targets_dict'+str(i)]
        rcnn_loss = 0
        for i in range(6):
            ...
            if 'targets_dict_p'+str(i) in self.forward_ret_dict:
                ...
                rcnn_loss += 0.5*rcnn_loss_reg
        #print("rcnn_loss_cls",rcnn_loss_cls.item()," rcnn_loss_reg",rcnn_loss_reg.item())
