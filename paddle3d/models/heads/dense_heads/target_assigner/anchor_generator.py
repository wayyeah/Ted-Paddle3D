    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            x_shifts = paddle.arange(
                self.anchor_range[0] + x_offset, self.anchor_range[3] + 1e-5, step=x_stride, dtype=paddle.float32,
            )
            y_shifts = paddle.arange(
                self.anchor_range[1] + y_offset, self.anchor_range[4] + 1e-5, step=y_stride, dtype=paddle.float32,
            )
            z_shifts = paddle.to_tensor(anchor_height, dtype=x_shifts.dtype)


            num_anchor_size, num_anchor_rotation = anchor_size.__len__(), anchor_rotation.__len__()
            anchor_rotation = paddle.to_tensor(anchor_rotation,dtype=x_shifts.dtype)
            anchor_size = paddle.to_tensor(anchor_size,dtype=x_shifts.dtype)
            x_shifts, y_shifts, z_shifts = paddle.meshgrid([
                x_shifts, y_shifts, z_shifts
            ])  # [x_grid, y_grid, z_grid]
            anchors = paddle.stack((x_shifts, y_shifts, z_shifts), axis=-1)  # [x, y, z, 3]
            anchors = anchors.unsqueeze(3).tile([1, 1, 1, anchor_size.shape[0], 1])
            anchor_size = anchor_size.reshape([1, 1, 1, -1, 3]).tile([*anchors.shape[0:3], 1, 1])
            anchors = paddle.concat((anchors, anchor_size), axis=-1)
            anchors = anchors.unsqueeze(4).tile([1, 1, 1, 1, num_anchor_rotation, 1])
            anchor_rotation = anchor_rotation.reshape([1, 1, 1, 1, -1, 1]).tile([*anchors.shape[0:3], num_anchor_size, 1, 1])
            anchors = paddle.concat((anchors, anchor_rotation), axis=-1)  # [x, y, z, num_size, num_rot, 7]
