...
def rotate_points_along_z(points, angle):
    ...
    """
    if type(angle) is not paddle.Tensor:
        angle = paddle.to_tensor(angle)
    if type(points) is not paddle.Tensor:
        points = paddle.to_tensor(points)
    cosa = paddle.cos(angle).astype('float32')
    sina = paddle.sin(angle).astype('float32')
...
def generate_voxel2pinds(sparse_tensor_shape, sparse_tensor_indices):
    ...
    output_shape_tensor = paddle.to_tensor(output_shape)
    assert (sparse_tensor_indices >= 0).all().numpy() and (sparse_tensor_indices < output_shape_tensor).all().numpy(), "Index out of bounds!"


    '''return paddle.scatter_nd(
        index=sparse_tensor_indices.astype('int32'),
        updates=point_indices ,
        shape=output_shape)  '''
    return paddle.scatter_nd(
        index=sparse_tensor_indices.astype('int32'),
