...
from paddle3d.datasets import augmentor_utils
import copy
import cv2
import numpy as np
...
__all__ = [
    ...
    "RandomScaleImageMultiViewImage","X_TRANS"
]
@manager.TRANSFORMS.add_component
class X_TRANS(TransformABC):
    ...
    def world_rotation(self, data_dict=None, config=None):
        ...
        if rot_factor.shape==[1,1]:
            rot_factor = rot_factor[0]
        
        if 'points' in data_dict:
            points = data_dict['points']
            if self.backward_flag:
                points[:,0:3] = box_utils.rotate_points_along_z(points[np.newaxis, :, 0:3], -rot_factor)[0]
            else:
                points[:, 0:3] = box_utils.rotate_points_along_z(points[np.newaxis, :, 0:3], rot_factor)[0]
        ...
        if 'boxes' in data_dict:
            ...
            if len(boxes_lidar):
                if self.backward_flag:
                    boxes_lidar[:, 0:3] = box_utils.rotate_points_along_z(boxes_lidar[np.newaxis, :, 0:3], -rot_factor)[0]
    ...
    def world_scaling(self, data_dict=None, config=None):
        ...
        scale_factor = data_dict['transform_param'][2].astype("float32")
    ...
    def input_transform(self, data_dict, trans_boxes=False):
        ...
        #data_dict['points']=data_dict['data']
        self.rot_num = data_dict['rot_num']
        src_points = copy.deepcopy(data_dict['points'])
        if trans_boxes and 'gt_boxes' in data_dict:
    ...
    def __call__(self, sample: Sample):
        ...
        if sample.rot_num>1:
            return self.input_transform(sample,True)
        else:
